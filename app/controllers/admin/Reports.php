...
class Reports extends MY_Controller
{
    ...
    public function getSalesReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status, DATE_FORMAT(due_date, '%Y-%m-%d %T') as due_date, {$this->db->dbprefix('sales')}.id as id", false)
                ->from('sales')
    ...
    public function getSalesReport1($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            $this->db
                ->select("date, sales.id as id, username, customer, companies.city as city, products.code as sku, products.name as iname, sale_items.product_unit_code as unit, sale_items.quantity as quantity, sale_items.unit_price as price, products.cf1/100*price as comision, {$this->db->dbprefix('sales')}.id as id", false)
                ->from('products')
            ...
            if ($start_date) {
                ...
                $fecha_inicio = $fecha_actual->format('Y-m-d') . ' 00:00:00';
                $fecha_fin = $fecha_actual->format('Y-m-d') . ' 23:59:59';
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('sku'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('products'));
                ...
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('comision'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->username);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->customer);
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->city);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->sku);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->price);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $data_row->comision);
                    //$total   += $data_row->grand_total;
                    $paid    += $data_row->quantity;
                    $balance += $data_row->price;
                    $comision += $data_row->comision;
                    $row++;
                }
                ...
                $this->excel->getActiveSheet()->SetCellValue('J' . $row, $comision);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, sales.id, username, customer, companies.city as city, products.code as sku, products.name as iname, sale_items.product_unit_code as unit_code, sale_items.quantity as quantity, sale_items.unit_price as price, products.cf1/100*price as comision, {$this->db->dbprefix('sales')}.id as id", false)
                ->from('products')
                ->join('sale_items', 'sale_items.product_id=products.id', 'left')
                ->join('sales', 'sales.id=sale_items.sale_id', 'left')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
